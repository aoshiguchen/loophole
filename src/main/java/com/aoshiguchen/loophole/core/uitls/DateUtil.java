...
public class DateUtil {
	 ...
	 * @return
	 */
	public static Date getBeforeDayBegin(){
		Date today = DateUtil.getDayBeforeOrNext(new Date(), -1);
	...
	/**
	 * 获取两个时间之间经历的时间 返回(yyyy-MM-dd)
	 *
	 * @param beginDate 开始时间
	 * @param endDate   结束时间
	 */
	public static List<String> getDateList(Date beginDate, Date endDate) {
		LocalDate beginDates = LocalDate.parse(DateUtil.format(beginDate, "yyyy-MM-dd"));
		LocalDate endDates = LocalDate.parse(DateUtil.format(endDate, "yyyy-MM-dd"));
		return Stream.iterate(beginDates, localDate -> localDate.plusDays(1)).limit(ChronoUnit.DAYS.between(beginDates, endDates) + 1).map(LocalDate::toString).collect(Collectors.toList());
	}
